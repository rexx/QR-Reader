...
這條慣例裡的「舊」**必須指明相對於什麼事件**，否則無法套用。 這裡的基準點是 **Service Worker 上線**（本專案是 `a93696e`），不是「早於當前 HEAD」。`public/manifest.json` 在更早的 `ec43f51` 就存在，所以本專案在沒有 SW 的那段期間**已經可以被加到主畫面**——那種安裝才是這條慣例講的對象。SW 上線之後才裝的，即使裝了好幾版也不算。

**這條目前零驗證。** 沒有人在真機上確認過 pre-SW 安裝是否真的會失敗。要驗只有一次機會：在刪除那個安裝之前先試離線冷啟動。不成立的價值更大——那表示這條該降級。

2026-08-23 的真機測試**沒有回答這個問題**。測試裝置上確實存在一個早於 SW 的安裝，但那次成功的離線冷啟動是發生在該安裝上、還是刪除重裝之後，已無法確認。所以那次結果證明了離線冷啟動可用，但對這條慣例不構成任何證據。
...
**這一節刻意只寫結論，不複述一手文件的推理或樣本數字。** 該文件已經改寫過一次並撤回了自己先前的因果敘述，本節上一版因為複製了那條推理而一起失效。要細節請直接讀 Cozy-Pocket [`docs/app-icon-ios-liquid-glass.md`](https://github.com/rexx/Cozy-Pocket/blob/main/docs/app-icon-ios-liquid-glass.md)，不要從這裡二手理解。

**QR Reader 的 icon 拿不到 iOS 26 的 Liquid Glass 處理。** icon 是滿版不透明（100% 不透明像素）；web clip 只能交單張點陣圖、拿不到原生的分層 `.icon`，所以 alpha 是 iOS 推導前景的唯一線索，滿版不透明等於宣告整張圖都是完稿。

**這是確定的取捨，不是未知風險，而且不影響任何功能**——只是主畫面 icon 沒有系統的邊緣立體處理。深色是自己畫進去的，不需要任何系統生成行為來解釋。想換取那個效果得改成透明底線稿，代價是 favicon 與 maskable 仍需各自烤底。

要改 icon 時去讀一手文件，特別注意兩件事：**什麼條件會觸發加工目前仍有未解的部分**（覆蓋率門檻的真因有三個競爭假說尚未分離），以及**驗證只能實機**（每次驗證前要先移除主畫面舊圖示，iOS 會快取 web clip icon；並比對 Light／Dark／Clear／Tinted 四種外觀）。
