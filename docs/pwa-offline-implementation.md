...
**必須指明「舊」是相對於什麼事件**，否則這條無法套用。 這裡的基準點是 **Service Worker 上線**（本專案是 `a93696e`），不是「早於當前 HEAD」。`public/manifest.json` 在更早的 `ec43f51` 就存在，所以本專案在沒有 SW 的那段期間**已經可以被加到主畫面**——那種安裝才是這條慣例講的對象。SW 上線之後才裝的，即使裝了好幾版也不算。
