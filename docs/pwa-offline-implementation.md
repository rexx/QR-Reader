...
**一手來源**：Cozy-Pocket [`docs/app-icon-ios-liquid-glass.md`](https://github.com/rexx/Cozy-Pocket/blob/main/docs/app-icon-ios-liquid-glass.md)（106 行，5 個真機樣本）。下面只摘與本專案有關的部分，需要細節請讀原文，不要引用這裡的轉述。

| 主張 | 證據等級 |
|---|---|
| 滿版不透明 icon 拿不到 Liquid Glass | **真機實測**。樣本表第一列：0% 透明 → ❌ |
| 填滿主體內部會失效，且與填色的 alpha 無關 | **真機實測**。半透明 20% 與不透明深青分別測，結果相同 |
| 透明像素比例門檻（76.3% / 86.5% 有效、62% 失效） | **真機實測**，但只是代理指標 |
| 62–76% 之間的行為 | **未驗證區** |
| iOS 會自行生成背景 | **真機實測**。對照 PWA 的 icon 四角與中心 alpha 皆為 0，主畫面實拍卻是深底加邊緣光影 |
| iOS 依什麼決定生成的**底色** | **無人驗證**。一手文件完全沒提。不要斷言「從 icon 自身顏色反推」，也不要斷言「依系統淺色/深色模式」 |

**因果是不要填滿主體內部，比例只是副產品。** 一手文件明講：先看有沒有面狀填色，比例當第二道防線，不要倒過來。小造型即使填滿內部仍可能通過 76.3% 門檻卻照樣失效。

**QR Reader 確定拿不到 Liquid Glass。** icon 是滿版不透明（100% 不透明像素），對應樣本表第一列的失效條件。這是個**確定的取捨，不是未知風險**——深色是自己畫進去的，不需要任何系統生成行為來解釋。想換取這個效果的路徑是改成描邊線稿、不要面狀填色，代價是 favicon 與 maskable 仍需自己烤底。

**設計限制若要改 icon**：圖形不能依賴背景（沒有保證存在的底色，造型要由不透明描邊與實心色塊撐起）；不要把玻璃、陰影、反射烘進 PNG（會破壞四種外觀的系統適配，可能出現雙重高光）。

**驗證程序**（只能實機，模擬器與桌面瀏覽器都不行）：

1. **每次驗證前先移除主畫面舊圖示再重新加入。** iOS 會快取 web clip icon，不移除會看到舊圖而誤判成「沒生效」。這與 §5.2 的舊安裝問題是兩個不同的快取，但補救動作剛好相同
2. 比對 Light / Dark / Clear / Tinted 四種外觀，看邊緣是否出現立體處理、四種下是否都清楚無雙重高光
...
**2026-08-23 真機結果**：iPhone 主畫面安裝後，飛航模式冷啟動成功進入 app，相機正常啟動（§6.3 通過、§6.4 的相機部分通過）。§6.5 離線降級與 §6.6 恢復連線尚未在真機逐項走過。
