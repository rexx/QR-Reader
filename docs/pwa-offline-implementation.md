...
### 5.3 Icon 在主畫面上的呈現（證據等級分明）

這一節的說法在專案之間轉手過幾次，每轉一次就容易被升級成事實，所以先分級。

| 主張 | 證據等級 |
|---|---|
| 近白、低飽和（0.08）的透明線稿在 iOS 主畫面上幾乎看不見 | **真機直接觀察**（mini-sudoku，使用者截圖） |
| 各 icon 檔案的透明度與飽和度數值 | **檔案層量測**，可複現 |
| 滿版不透明 icon 會被排除在 iOS Liquid Glass 處理之外 | **未經證實的慣例**。源自 Cozy-Pocket `scripts/generate-icons.mjs:20-22` 的註解；mini-sudoku 沿用未驗證，本專案也未在真機複驗——整條鏈上沒有人驗過 |
| 透明度 76.3% 門檻、飽和度 0.5 門檻 | 同上，是對「唯一真的看到過的那次失效」設的絆線，不是 iOS 的模型 |

**機制不要斷言。** 「iOS 從 icon 自身顏色反推底色」是用兩個資料點擬合出來的推論，其中一個還是口述的。有一個同樣說得通的競爭假說：iOS 依系統淺色/深色模式決定底色，而那個中亮度青色只是在兩種底下都讀得出來。單一截圖分不開這兩者。

**真正該當通則的是這句**：讓線稿在任何底色下都可讀，勝過預測 iOS 會給哪種底色。做法是把線稿合成到白、iOS 淺灰、近黑三種底上各看一次——這個檢查可複現，而且不依賴任何關於 iOS 行為的假說。

QR Reader 目前的狀態：icon 是滿版不透明深色圖（不透明像素 100%、平均飽和度 0.623、平均明度 0.239）。**深色是自己畫進去的**，不需要任何 iOS 生成行為來解釋它在主畫面上呈深色。因此上面那兩個門檻對本專案都不適用——別拿滿版 icon 去對透明度或飽和度門檻，量錯維度會得到「過了」的假結論。

若之後改成透明線稿，走上面那條三底色可讀性檢查，不要只依賴門檻。
